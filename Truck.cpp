...
void Truck::playMusic(Point cameraPos) {
    float distance = position.dist(cameraPos);

    if (isMusicPlaying) {
        PlaySoundA(NULL, 0, 0);
        isMusicPlaying = false;
        cout << "Music stopped by user." << endl;
    }
    else {
        if (distance < 500.0f) {
            // SND_LOOP keeps it playing until we tell it to stop
            PlaySoundA(this->musicSoundPath, NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
            isMusicPlaying = true;
            cout << "Music started." << endl;

            // Visual Flash (Optional: separate this into a render function)
            //renderFlash();
        }
    }
}

void Truck::checkMusicDistance(Point cameraPos) {
    if (!isMusicPlaying) return;

    if (position.dist(cameraPos) > 100.0f) {
        PlaySoundA(NULL, 0, 0); // Stop sound
        isMusicPlaying = false;
        cout << "Music stopped: Player moved too far." << endl;
    }
}
...
void Truck::draw(float r, float g, float b) {
    ...
    glPushMatrix();
    glTranslatef(cabL / 3, cabH / 2.8, -cabW / 4);
    glRotatef(300, 0, 1, 0);
    glRotatef(-20, 0, 0, 1);
    ...
    gluCylinder(quad, 0.35f, 0.25f, 1.5f, 20, 1);
    ...
    glRotatef(-85, 0, 1, 0);
    glColor3f(0.05f, 0.05f, 0.05f);
    gluCylinder(quad, 0.05f, 0.04f, 0.8f, 8, 1);
    glTranslatef(0, 0, 0.8f);
    gluSphere(quad, 0.07f, 12, 12);
    glPopMatrix();
    ...
    glRotatef(85, 0, 1, 0);
    glColor3f(0.05f, 0.05f, 0.05f);
    gluCylinder(quad, 0.05f, 0.04f, 0.7f, 8, 1);
    ...
    glPushMatrix();
    glRotatef(steerAngle, 0, 0, 1);

    glColor3f(0.02f, 0.02f, 0.02f);
    glutSolidTorus(0.18f, 1.8f, 20, 40);
    glColor3f(0.6f, 0.6f, 0.6f);
    glutSolidTorus(0.02f, 1.8f, 10, 40);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        glColor3f(0.15f, 0.15f, 0.15f);
        glBegin(GL_QUADS);
        glVertex3f(0.4f, 0.18f, 0.05f);
        glVertex3f(1.15f, 0.28f, 0);
        glVertex3f(1.15f, -0.28f, 0);
        glVertex3f(0.4f, -0.18f, 0.05f);
        glEnd();
        glColor3f(0.3f, 0.3f, 0.3f);
        glBegin(GL_LINES);
        for (float lx = 0.5f; lx < 1.0f; lx += 0.1f) {
            glVertex3f(lx, 0.15f, 0.06f); glVertex3f(lx, 0.22f, 0.06f);
            glVertex3f(lx, -0.15f, 0.06f); glVertex3f(lx, -0.22f, 0.06f);
        }
        glEnd();
        glPopMatrix();
    ...
    glRotatef(90, 1, 0, 0);
    glColor3f(0.1f, 0.1f, 0.1f);
    gluDisk(quad, 0, 0.5f, 24, 1);
    glTranslatef(0, 0, -0.15f);
    glColor3f(0.08f, 0.08f, 0.08f);
    gluSphere(quad, 0.35f, 24, 24);
    glTranslatef(0, 0, -0.36f);
    glColor3f(0.9f, 0.9f, 0.9f); // Logo
    gluDisk(quad, 0, 0.06f, 12, 1);
    glPopMatrix();
    glPopMatrix();

    gluDeleteQuadric(quad);
    glPopMatrix(); // End Steering System
    ...
    glPopMatrix(); // End Cabin Translate
    ...
    // --- LIGHTS ---
    // (Existing light code remains same)
