...
void Controller::keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        // --- Camera Movement (Always WASD) ---
    case 'w': case 'W': cameraRef->Fly(5.0f); break;
    case 's': case 'S': cameraRef->Fly(-5.0f); break;
    case 'a': case 'A': cameraRef->RotateYaw(-0.02f); break;
    case 'd': case 'D': cameraRef->RotateYaw(0.02f); break;

    case 'm': g_darkMode = !g_darkMode; break;
    case 'e': buldingRef->toggleDoor(); break;
    case 'n': cameraRef->openNearestDoor(); break;
